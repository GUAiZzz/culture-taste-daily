...
The follow-up owner review removes the Analog channel preview entirely because article selection already exists below the cover. The homepage opens directly on the current cover, without channel buttons, preview labels or a swipe cue. Retired channel URL parameters and session values no longer replace the cover. The fixed television frame, grain, reduced-motion behavior and full article list remain.
...
The masthead wordmark uses one responsive type scale across the homepage, archive, issue and article pages in all three rooms. Typography and spacing are shared; only room colour and material treatment vary. Daily has the same desktop visibility breakpoint everywhere. Legacy original editions remain preserved.
